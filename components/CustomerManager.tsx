...
const CustomerManager: React.FC = () => {
          ...
          try {
              ...
              if (data && data.display_name) {
                  setFormData(prev => ({ 
                      ...
                      isAddressVerified: true // Mark as verified via GPS
                  }));
                  toast.success("Đã lấy địa chỉ hiện tại!", { icon: '📍' });
              } else {
                  toast.error("Không tìm thấy địa chỉ");
              }
          } catch (e) {
              toast.error("Lỗi lấy địa chỉ từ bản đồ");
          } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await storageService.upsertCustomer({
        ...
        totalOrders: 0,
        updatedAt: Date.now(),
        isAddressVerified: formData.isAddressVerified || false
    });
    setFormData({ name: '', phone: '', address: '', priorityScore: 999, isAddressVerified: false });
    setIsAdding(false);
  ...
  const handleInlineUpdate = async (customer: Customer, field: keyof Customer, value: any) => { 
      ...
      const updateData: Partial<Customer> = { [field]: value, updatedAt: Date.now() };
      
      // If address is changed manually, we must UN-VERIFY it because it might no longer match the pinned location
      if (field === 'address') {
          updateData.isAddressVerified = false;
      }

      await storageService.upsertCustomer({ ...customer, ...updateData }); 
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2 relative">
                      <input 
                          ...
                          onChange={e => setFormData({ ...formData, address: e.target.value, isAddressVerified: false })}
                          className={`w-full p-3 bg-gray-50 border-2 rounded-xl font-bold outline-none pr-20 ${formData.isAddressVerified ? 'border-green-500 focus:border-green-600 bg-green-50' : 'border-gray-200 focus:border-black'}`}
                      ...
                      <div className="absolute right-2 top-2 flex gap-1">
                          <button 
                              ...
                              className={`w-9 h-9 rounded-lg flex items-center justify-center transition-all ${isGettingLocation ? 'bg-gray-100 text-gray-400' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}
                              title="Ghim vị trí hiện tại"
                          >
                              <i className={`fas ${isGettingLocation ? 'fa-circle-notch fa-spin' : 'fa-map-marker-alt'}`}></i>
                          </button>
                      </div>
                  </div>
                  {formData.isAddressVerified && (
                      <div className="md:col-span-2 text-[10px] text-green-600 font-bold flex items-center gap-1">
                          <i className="fas fa-check-circle"></i> Đã ghim toạ độ chính xác từ GPS
                      </div>
                  )}
                  ...
                  >
                      <div className="p-5">
                          ...
                          <div className="space-y-2 mb-4">
                              ...
                              <div className={`flex items-start gap-3 p-2 rounded-xl border border-transparent transition-colors ${customer.isAddressVerified ? 'bg-green-50/50' : 'bg-gray-50 hover:border-eco-100'}`}>
                                  {customer.isAddressVerified ? (
                                      <i className="fas fa-map-marker-alt text-green-500 text-xs w-4 text-center mt-1" title="Đã ghim vị trí chính xác"></i>
                                  ) : (
                                      <i className="fas fa-map-marker-alt text-red-500 text-xs w-4 text-center mt-1"></i>
                                  )}
                                  <textarea 
                                      className={`text-[11px] font-bold bg-transparent border-none p-0 outline-none flex-grow resize-none leading-snug ${customer.isAddressVerified ? 'text-green-800' : 'text-gray-500'}`}
                                      rows={2}
                                      defaultValue={customer.address}
                                      placeholder="Chưa có địa chỉ"
                                      onBlur={(e) => handleInlineUpdate(customer, 'address', e.target.value)}
                                  />
                                  {customer.isAddressVerified && <i className="fas fa-check-circle text-green-500 text-[10px] mt-1"></i>}
                              </div>
                          </div>
