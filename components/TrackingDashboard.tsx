...
const TrackingDashboard: React.FC = () => {
  ...
  const newCustomerMap = useMemo(() => {
      ...
      orders.forEach(o => {
          const key = o.id; 
          map[key] = storageService.isNewCustomer(o.customerPhone, o.address);
      });
      return map;
  }, [orders, customers]); 
  ...
  const handlePrintConfirm = async (type: 'LIST' | 'INVOICE') => {
      ...
      try {
          if (type === 'LIST') {
              await pdfService.generateCompactList(ordersToPrint, batchName);
          } else {
              await pdfService.generateInvoiceBatch(ordersToPrint, batchName);
          }
  ...
  const prepareSplitPrint = (subset: Order[]) => {
      setOrdersToPrint(subset);
      setShowBatchSplitModal(false);
      setShowPrintTypeModal(true);
  };
  ...
  const handleReconcileFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
          ...
          if (result.matchedOrders.length === 0) {
              toast('Không tìm thấy giao dịch nào khớp.', { icon: '🔍' });
          } else {
              toast.success(`Tìm thấy ${result.matchedOrders.length} giao dịch khớp!`);
          }
      } catch (error) {
          console.error(error);
  ...
  const confirmReconciliation = async () => {
      ...
      const promises = reconcileResult.matchedOrders.map(order => 
          storageService.updatePaymentVerification(order.id, true, { name: order.customerName })
      );
  ...
  return (
    <div className="animate-fade-in pb-20">
      ...
      <div className={`${isCompactMode ? 'bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col divide-y divide-gray-100' : (sortBy === 'ROUTE' ? 'grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4' : 'grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4')} mt-1`}>
        {visibleOrders.length === 0 ? (<div className="col-span-full flex flex-col items-center justify-center py-20 text-gray-300"><i className="fas fa-box-open text-6xl mb-4 opacity-50"></i><p className="text-lg font-medium">Không tìm thấy đơn hàng nào</p></div>) : (visibleOrders.map((order, index) => (<div key={order.id} data-index={index} draggable={sortBy === 'ROUTE'} onDragStart={(e) => handleDragStart(e, index)} onDragEnter={(e) => handleDragEnter(e, index)} onDragEnd={handleDragEnd} onDragOver={(e) => e.preventDefault()} className={`relative transition-all duration-200 order-row`}><div className="flex-grow h-full">
            <OrderCard 
                order={order} 
